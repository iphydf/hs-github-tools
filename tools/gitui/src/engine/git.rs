...
impl RealGit {
    ...
    fn git_cmd(&self) -> std::process::Command {
        let mut cmd = std::process::Command::new(crate::git_bin());
        cmd.arg("-C")
            .arg(&self.path)
            .env_remove("GIT_DIR")
...
impl Git for RealGit {
    ...
    fn get_branches(
        ...
        let pr_map = super::github::fetch_open_prs(&self.path);
        ...
        for (i, b) in filtered_branch_data.into_iter().enumerate() {
            ...
            let pr = pr_map.get(&b.name).cloned();
    ...
    fn run_command(&self, args: &[String]) -> anyhow::Result<bool> {
        // Subprocess stdout (e.g., "Fast-forward", "Updating x..y") is
        // human-readable progress, not structured data — route it to stderr
        // so it doesn't pollute the caller's stdout when emitting JSON.
        let mut child = self
            .git_cmd()
            .args(args)
        ...
        if let Some(mut stdout) = child.stdout.take() {
            std::thread::spawn(move || {
                let _ = std::io::copy(&mut stdout, &mut std::io::stderr().lock());
            });
        }
